...
const Header = () => {
  ...
  return (
    ...
    >
      ...
      {/* Menu */}
      <ul
        className={`${isMenuOpen ? "h-64 opacity-100 py-4" : "h-0 opacity-0"
      ...
      >
        {["Home", "About", "Skills", "Projects"].map((item) => (
          <li key={item} className="w-full text-center md:w-auto">
            ...
            >
              ...
              <span className="absolute left-1/2 bottom-1 transform -translate-x-1/2 w-0 group-hover:w-1/2 h-[2px] bg-cyan-400 transition-all duration-300"></span>
            </a>
          </li>
        ))}
        ...
        <li className="w-full text-center md:w-auto mt-2 md:mt-0">
          <a
            href="#contact"
            className="inline-block py-2 px-6 bg-cyan-500 text-white rounded-full text-sm font-semibold shadow-lg hover:bg-cyan-600 transition-all duration-300"
            onClick={() => setIsMenuOpen(false)}
          >
            Contact
          </a>
